...
import { getMostPopularAnime } from '../api/index';
import { getAnimeByCategory } from '../api';
  ...
  return (
    <DashboardMain>
      {loading ?
        ...
        <FilterContainer>
            <Topic>Most Popular
              <Link to={`/showanimes/mostpopular`} style={{ textDecoration: "none" }}>
                <Span>Show All</Span>
              </Link>
            </Topic>
            <Animes>
              {mostPopular.slice(0, 10).map((anime) => (
                <AnimeCard anime={anime} user={user} setSignInOpen={setSignInOpen} />
              ))}
            </Animes>
          </FilterContainer>
          {currentUser && user?.animes?.length > 0 &&
            <FilterContainer box={true}>
              ...
              <Animes>
                {user?.animes.slice(0, 10).map((anime) => (
                  <AnimeCard anime={anime} user={user} setSignInOpen={setSignInOpen} />
                ))}
              </Animes>
            </FilterContainer>
